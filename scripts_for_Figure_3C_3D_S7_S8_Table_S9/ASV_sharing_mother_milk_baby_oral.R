################################################################################
##### ASVs transmission analysis between maternal milk and infant oral samples
...
# 16S V3-V4 results
genera_counts_16S <- read.delim("16S/Mother_Milk-Baby_Oral/Genera_counts.txt", header = T, check.names = F) 
genera_in_pairs_16S <- readRDS("16S/Mother_Milk-Baby_Oral/Genera_counts_pairs.rds")
matching_ASVs_16S <- readRDS("16S/Mother_Milk-Baby_Oral/ASVs_shared_genera.rds")

# 16S-ITS-23S results
genera_counts_16S23S <- read.delim("16S23S/Mother_Milk-Baby_Oral/Genera_counts.txt", header = T, check.names = F) 
genera_in_pairs_16S23S <- readRDS("16S23S/Mother_Milk-Baby_Oral/Genera_counts_pairs.rds")
matching_ASVs_16S23S <- readRDS("16S23S/Mother_Milk-Baby_Oral/ASVs_shared_genera.rds")
...
#################################################
#A. 16S_23S - Mother Milk vs Baby Oral
#################################################

setwd("~/Desktop/PhD/Projects/Johanne methods paper/Final_paper_version/16S23S/Mother_Milk-Baby_Oral/")
...
Gemella_in_pairs <- names(grep("Gemella", genera_in_pairs_16S23S, value = T))
Pauljensenia_in_pairs <- names(grep("Pauljensenia", genera_in_pairs_16S23S, value = T))
Rothia_in_pairs <- names(grep("Rothia", genera_in_pairs_16S23S, value = T))
Streptococcus_in_pairs <- names(grep("Streptococcus", genera_in_pairs_16S23S, value = T))
Veillonella_in_pairs <- names(grep("Veillonella", genera_in_pairs_16S23S, value = T))

# Get the contigency table and the pairs in which we observe PSEs. Add results to final_PSE_data
PSE_Gemella_16S23S <- exact_PSE_analysis(SNP_distances_16S23S$Gemella_16S23S, matching_ASVs_16S23S)
final_PSE_data[rownames(final_PSE_data) %in% Gemella_in_pairs,"Gemella_16S23S" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Gemella_16S23S$Pairs_with_PSEs),"Gemella_16S23S" ] <- "PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Gemella_16S23S$Unrelated_pairs_without_PSE),"Gemella_16S23S" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Gemella_16S23S$Unrelated_pairs_with_PSE),"Gemella_16S23S" ] <- "PSE"

PSE_Pauljensenia_16S23S <- exact_PSE_analysis(SNP_distances_16S23S$Pauljensenia_16S23S, matching_ASVs_16S23S)
final_PSE_data[rownames(final_PSE_data) %in% Pauljensenia_in_pairs,"Pauljensenia_16S23S" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Pauljensenia_16S23S$Pairs_with_PSEs),"Pauljensenia_16S23S" ] <- "PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Pauljensenia_16S23S$Unrelated_pairs_without_PSE),"Pauljensenia_16S23S" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Pauljensenia_16S23S$Unrelated_pairs_with_PSE),"Pauljensenia_16S23S" ] <- "PSE"

PSE_Rothia_16S23S <- exact_PSE_analysis(SNP_distances_16S23S$Rothia_16S23S, matching_ASVs_16S23S)
final_PSE_data[rownames(final_PSE_data) %in% Rothia_in_pairs,"Rothia_16S23S" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Rothia_16S23S$Pairs_with_PSEs),"Rothia_16S23S" ] <- "PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Rothia_16S23S$Unrelated_pairs_without_PSE),"Rothia_16S23S" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Rothia_16S23S$Unrelated_pairs_with_PSE),"Rothia_16S23S" ] <- "PSE"
...
PSE_Veillonella_16S23S <- exact_PSE_analysis(SNP_distances_16S23S$Veillonella_16S23S, matching_ASVs_16S23S)
final_PSE_data[rownames(final_PSE_data) %in% Veillonella_in_pairs,"Veillonella_16S23S" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Veillonella_16S23S$Pairs_with_PSEs),"Veillonella_16S23S" ] <- "PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Veillonella_16S23S$Unrelated_pairs_without_PSE),"Veillonella_16S23S" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Veillonella_16S23S$Unrelated_pairs_with_PSE),"Veillonella_16S23S" ] <- "PSE"

# Save contingency tables in list
cont_tables_16S23S <- list(PSE_Gemella_16S23S$Contingency_table, PSE_Pauljensenia_16S23S$Contingency_table,
                           PSE_Rothia_16S23S$Contingency_table, PSE_Streptococcus_16S23S$Contingency_table,
                           PSE_Veillonella_16S23S$Contingency_table)

names (cont_tables_16S23S) <- names_shared_genera

#################################################
#B. V3V4 - Mother Milk vs Baby Oral 
#################################################

setwd("~/Desktop/PhD/Projects/Johanne methods paper/Final_paper_version/16S/Mother_Milk-Baby_Oral/")
...
# Pairs in which each genus is detected
Gemella_in_pairs <- names(grep("Gemella", genera_in_pairs_16S, value = T))
Pauljensenia_in_pairs <- names(grep("Pauljensenia", genera_in_pairs_16S, value = T))
Rothia_in_pairs <- names(grep("Rothia", genera_in_pairs_16S, value = T))
Streptococcus_in_pairs <- names(grep("Streptococcus", genera_in_pairs_16S, value = T))
Veillonella_in_pairs <- names(grep("Veillonella", genera_in_pairs_16S, value = T))

# Get the contigency table and the pairs in which we observe PSEs. Add results to final_PSE_data
PSE_Gemella_V3V4 <- exact_PSE_analysis(SNP_distances_V3V4$Gemella_V3V4, matching_ASVs_16S)
final_PSE_data[rownames(final_PSE_data) %in% Gemella_in_pairs,"Gemella_V3V4" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Gemella_V3V4$Pairs_with_PSEs),"Gemella_V3V4" ] <- "PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Gemella_V3V4$Unrelated_pairs_without_PSE),"Gemella_V3V4" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Gemella_V3V4$Unrelated_pairs_with_PSE),"Gemella_V3V4" ] <- "PSE"

PSE_Pauljensenia_V3V4 <- exact_PSE_analysis(SNP_distances_V3V4$Pauljensenia_V3V4, matching_ASVs_16S)
final_PSE_data[rownames(final_PSE_data) %in% Pauljensenia_in_pairs,"Pauljensenia_V3V4" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Pauljensenia_V3V4$Pairs_with_PSEs),"Pauljensenia_V3V4" ] <- "PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Pauljensenia_V3V4$Unrelated_pairs_without_PSE),"Pauljensenia_V3V4" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Pauljensenia_V3V4$Unrelated_pairs_with_PSE),"Pauljensenia_V3V4" ] <- "PSE"

PSE_Rothia_V3V4 <- exact_PSE_analysis(SNP_distances_V3V4$Rothia_V3V4, matching_ASVs_16S)
final_PSE_data[rownames(final_PSE_data) %in% Rothia_in_pairs,"Rothia_V3V4" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Rothia_V3V4$Pairs_with_PSEs),"Rothia_V3V4" ] <- "PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Rothia_V3V4$Unrelated_pairs_without_PSE),"Rothia_V3V4" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Rothia_V3V4$Unrelated_pairs_with_PSE),"Rothia_V3V4" ] <- "PSE"
...
PSE_Veillonella_V3V4 <- exact_PSE_analysis(SNP_distances_V3V4$Veillonella_V3V4, matching_ASVs_16S)
final_PSE_data[rownames(final_PSE_data) %in% Veillonella_in_pairs,"Veillonella_V3V4" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Veillonella_V3V4$Pairs_with_PSEs),"Veillonella_V3V4" ] <- "PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Veillonella_V3V4$Unrelated_pairs_without_PSE),"Veillonella_V3V4" ] <- "no_PSE"
final_PSE_data[rownames(final_PSE_data) %in% unique(PSE_Veillonella_V3V4$Unrelated_pairs_with_PSE),"Veillonella_V3V4" ] <- "PSE"

# Save contingency tables in list
cont_tables_V3V4 <- list(PSE_Gemella_V3V4$Contingency_table, PSE_Pauljensenia_V3V4$Contingency_table,
                         PSE_Rothia_V3V4$Contingency_table, PSE_Streptococcus_V3V4$Contingency_table,
                         PSE_Veillonella_V3V4$Contingency_table)
...
write.table(final_PSE_data,"Result_tables/PSE_table_maternal_milk_infant_oral.txt", sep = "\t", row.names = T, quote = FALSE) 

# 16S V3-V4 results
saveRDS(cont_tables_V3V4, "16S/Mother_Milk-Baby_Oral/Contingency_tables_16S.rds")

# 16S-ITS-23S results
saveRDS(cont_tables_16S23S, "16S23S/Mother_Milk-Baby_Oral/Contingency_tables_16S23S.rds")
