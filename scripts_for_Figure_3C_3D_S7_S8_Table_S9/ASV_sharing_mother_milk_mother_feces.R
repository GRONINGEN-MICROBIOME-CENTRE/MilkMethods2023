################################################################################
##### ASVs transmission analysis between maternal milk and maternal fecal samples
...
# Defining functions
#********************
# A. Function to estimate the number of related and unrelated distances == or != 0.
  ...
  for (i in 1:nrow(SNP_distances)) {
    for (j in 1:ncol(SNP_distances)) {
      if(!is.na(SNP_distances[i,j])) {
        ...
        ASV1_sampletype <- gsub(".*_", "", gsub("(.*)_\\w+", "\\1", gsub("\\..*","",  ASV1_fullname)))
        ...
        ASV2_sampletype <- gsub(".*_", "", gsub("(.*)_\\w+", "\\1", gsub("\\..*","",  ASV2_fullname))) 
        ...
        if (SNP_distances[i,j] == 0) { #if the distance is below our threshold
          if (ASV1_sampletype != ASV2_sampletype) { #exclude comparisons from ASVs from the same sample origin
            if (ASV1_fullname != ASV2_fullname) { # exclude the same ASVs (obviously their distance is 0)
              if (ASV1_pair == ASV2_pair) { # if they are related (same pair_ID)
                ...
                unrelated_comp_with_PSE <-c(unrelated_comp_with_PSE, list(paste(paste(ASV1_sampletype,ASV1_pair, sep ="_"), 
                                                                                paste(ASV2_sampletype,ASV2_pair, sep ="_") , sep="_"))) #keep the unrelated pairs with PSE
                unrelated_comp_with_PSE <-c(unrelated_comp_with_PSE, list(paste(paste(ASV1_sampletype,ASV1_pair, sep ="_"), 
                                                                                paste(ASV2_sampletype,ASV2_pair, sep ="_") , sep="_"))) # all combinations
              }
            }
          }
          ...
          if (ASV1_sampletype != ASV2_sampletype) { #exclude comparisons from ASVs from the same sample origin
            if (ASV1_fullname != ASV2_fullname) { # exclude the same ASVs (obviously their distance is 0)
              if (ASV1_pair == ASV2_pair) { # if they are related (same pair_ID)
                ...
                unrelated_comp_without_PSE <-c(unrelated_comp_without_PSE, list(paste(paste(ASV1_sampletype,ASV1_pair, sep ="_"), 
                                                                                      paste(ASV2_sampletype,ASV2_pair, sep ="_") , sep="_"))) #keep the unrelated pairs without PSE
                unrelated_comp_without_PSE <-c(unrelated_comp_without_PSE, list(paste(paste(ASV1_sampletype,ASV1_pair, sep ="_"), 
                                                                                      paste(ASV2_sampletype,ASV2_pair, sep ="_") , sep="_"))) #all possible combinations 
              }
            }
          }
        }
...
# 16S V3-V4 results
genera_counts_16S <- read.delim("16S/Mother_Milk-Mother_Feces/Genera_counts.txt", header = T, check.names = F) 
genera_in_pairs_16S <- readRDS("16S/Mother_Milk-Mother_Feces/Genera_counts_pairs.rds")
matching_ASVs_16S <- readRDS("16S/Mother_Milk-Mother_Feces/ASVs_shared_genera.rds")

# 16S-ITS-23S results
genera_counts_16S23S <- read.delim("16S23S/Mother_Milk-Mother_Feces/Genera_counts.txt", header = T, check.names = F) 
genera_in_pairs_16S23S <- readRDS("16S23S/Mother_Milk-Mother_Feces/Genera_counts_pairs.rds")
matching_ASVs_16S23S <- readRDS("16S23S/Mother_Milk-Mother_Feces/ASVs_shared_genera.rds")

#################################################################################
# SNP distances analysis in ASVs of shared genera in maternal milk-fecal samples
#################################################################################
...
mothers_milk <- paste0("Milk_Pair",1:14)
mothers_feces <- paste0("Faeces_Pair", 1:14)

all_pairs <- expand.grid(mothers_milk, mothers_feces)
all_pairs_combined <- paste(all_pairs$Var1, all_pairs$Var2, sep="_")
related_pairs <- paste0( "Milk_Pair", 1:14, "_Faeces_Pair",1:14)
...
PSE_Streptococcus_16S23S <- exact_PSE_analysis_same_origin(SNP_distances_16S23S$Streptococcus_16S23S, matching_ASVs_16S23S)
...
cont_tables_16S23S <- list(PSE_Streptococcus_16S23S$Contingency_table)

names (cont_tables_16S23S) <- names_shared_genera

#################################################
#B. V3V4 - Mother Milk vs Mother Feces 
#################################################
...
# Get the contigency table and the pairs in which we observe PSEs. Add results to final_PSE_data
PSE_Streptococcus_V3V4 <- exact_PSE_analysis_same_origin(SNP_distances_V3V4$Streptococcus_V3V4, matching_ASVs_16S)
...
# Save contingency tables in list
cont_tables_V3V4 <- list(PSE_Streptococcus_V3V4$Contingency_table)
...
write.table(final_PSE_data,"Result_tables/PSE_table_maternal_milk_maternal_feces.txt", sep = "\t", row.names = T, quote = FALSE) 

# 16S V3-V4 results
saveRDS(cont_tables_V3V4, "16S/Mother_Milk-Mother_Feces/Contingency_tables_16S.rds")

# 16S-ITS-23S results
saveRDS(cont_tables_16S23S, "16S23S/Mother_Milk-Mother_Feces/Contingency_tables_16S23S.rds")
