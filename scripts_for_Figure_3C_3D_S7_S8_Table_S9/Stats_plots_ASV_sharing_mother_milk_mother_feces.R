################################################################################
##### ASVs transmission between maternal milk and fecal samples:  statistical analysis and plotting
...
# Read results files
final_PSE_data <- read.delim("Result_tables/PSE_table_maternal_milk_maternal_feces.txt", header = T, check.names = F) 
cont_tables_V3V4 <- readRDS("16S/Mother_Milk-Mother_Feces/Contingency_tables_16S.rds")
cont_tables_16S23S <- readRDS("16S23S/Mother_Milk-Mother_Feces/Contingency_tables_16S23S.rds")

###############################################################
# Generate PSE results table and test statistical significance 
###############################################################

# As we lack maternal fecal sample from pair 5 only in V3V4, we split final_PSE_data
final_PSE_data_V3V4 <- final_PSE_data 
final_PSE_data_V3V4$Streptococcus_16S23S <- NULL
final_PSE_data_16S23S <- final_PSE_data
final_PSE_data_16S23S$Streptococcus_V3V4 <- NULL

# From the V3V4 table, we remove the corresponding comparisons from the final_PSE_data
final_PSE_data_V3V4 <- final_PSE_data_V3V4[-(which(rownames(final_PSE_data) == "Pair5")),]
final_PSE_data_V3V4 <- final_PSE_data_V3V4[-grep("Faeces_Pair5", rownames(final_PSE_data_V3V4)),]

                                           
#******************************
# Test PSE differences in related vs unrelated pairs
#******************************
# Reformat the data tables for:
# (A) V3V4
final_PSE_data_V3V4$Pair_ID <- c(rownames(final_PSE_data_V3V4))

final_PSE_data_long_V3V4 <- final_PSE_data_V3V4 %>%
...
final_PSE_data_summary_V3V4 <- final_PSE_data_long_V3V4 %>%
  dplyr::group_by(Genus, Technology, Value, Relatedness) %>%
  dplyr::summarize(count = n()) %>%
  tidyr::pivot_wider(names_from = Value, values_from = count, values_fill = 0)

# (B) 16S23S
final_PSE_data_16S23S$Pair_ID <- c(rownames(final_PSE_data_16S23S))

final_PSE_data_long_16S23S <- final_PSE_data_16S23S %>%
  pivot_longer(cols = -c(Pair_ID, Relatedness), names_to = "Genus_Tech", values_to = "Value") %>%
  separate(Genus_Tech, into = c("Genus", "Technology"), sep = "_")

final_PSE_data_summary_16S23S <- final_PSE_data_long_16S23S %>%
  dplyr::group_by(Genus, Technology, Value, Relatedness) %>%
  dplyr::summarize(count = n()) %>%
  tidyr::pivot_wider(names_from = Value, values_from = count, values_fill = 0)
final_PSE_data_summary_16S23S$PSE <- c(0,0) #only when all PSE are 0

# Combine result tables
final_PSE_data_summary <- rbind(final_PSE_data_summary_V3V4, final_PSE_data_summary_16S23S)
...
for (i in 1:length(rownames(Fischer_tests))) {
  ...
  cont_table <- data.frame(rbind(c(genus_data[genus_data$Relatedness=="Related",7],
                                   genus_data[genus_data$Relatedness=="Related",4]),
                                 c(genus_data[genus_data$Relatedness=="Unrelated",7],
...
final_PSE_data_summary <- merge(final_PSE_data_summary, Fischer_tests, by.x="Genus_Tech", by.y=0) # no need to estimate FDR

# Reformat in long format
...
final_PSE_data_summary_long <- left_join(final_PSE_data_summary_long, total_count)
...
#******************************
# We subset the data frame with previous results per technology
...
# Select only "Related" comparisons and estimate FDR (BH) correction
sequencing_PSE_results <- sequencing_PSE_results[sequencing_PSE_results$Relatedness == "Unrelated",]
sequencing_PSE_results <- sequencing_PSE_results[,c("Genus", "Relatedness", "p_value")]
...
pdf('Plots/PSE_relatedness_V3V4_mother_milk_mother_feces.pdf', width=4.3, height=4)
...
pdf('Plots/PSE_relatedness_16S23S_mother_milk_mother_feces.pdf', width=4.3, height=4)
...
# Save results
########################
write.table(final_PSE_data_summary_long, file ="~/Desktop/PhD/Projects/Johanne methods paper/Final_paper_version/Result_tables/Mother_milk_Mother_Feces_final_PSE_results.txt", sep="\t")
write.table(sequencing_PSE_results, file ="~/Desktop/PhD/Projects/Johanne methods paper/Final_paper_version/Result_tables/Mother_milk_Mother_Feces_final_PSE_results_Seq_Technology.txt", sep="\t")
